...
describe('PokemonDetail.vuew', () => {
    ...
    beforeEach(() => {
        pokemon = {
            abilities: [
                { ability: { name: 'static', url: 'some-url' }, is_hidden: false, slot: 1 }
            ],
            base_experience: 112,
            cries: { latest: 'cry.mp3', legacy: 'legacy-cry.mp3' },
            froms: [{ name: 'pikachu', url: 'some-url' }],
            game_indices: [{ game_index: 1, version: { name: 'red', url: 'some-url' } }],
            height: 4,
            held_items: [],
            id: 25,
            is_default: true,
    ...
    it('Deve aplicar corretamente a cor de acordo com tipo do pokemon', () => {
        ...
        const nameElement = wrapper.find('.pokemon-info p');
        expect(nameElement.attributes('style')).toContain('color: rgb(248, 208, 48)')
    ...
    it ('Renderizando os tipos com a cor correta', () => {
        ...
        expect(typeElements[0].attributes('style')).toContain('background-color: rgb(248, 208, 48)');
